<?php
	...
	$sql="select cus_name from customers where username='".$username."'";
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
		...
		<link rel="stylesheet" type="text/css" href="css/ad_order_history.css"/>		
		<style type="text/css">
			a:link{
		...
		<div class="clearfix">
			<div class="layout">
				...
				<?php
					...
					{
						$query=mysqli_query($conn,"UPDATE orders SET status=1 WHERE order_ID='$orderID'");
					}
					if(isset($_POST['xoa']))
					{
						$query=mysqli_query($conn,"DELETE FROM orderlines WHERE order_ID='$orderID'");
						$query=mysqli_query($conn,"DELETE FROM orders WHERE order_ID='$orderID'");
				...
				?>
					<div class="b-order-info">
						...
						<p>Ngày đặt hàng: <?php echo $row1['date'];?></p>
							<form action="ad_order_detail.php?orderID=<?php echo $orderID;?>" method="POST">
								<ul class="search ">
									<?php
										if($row1['status']==0)
										{?>
											<div class="b-search--button">
												<button type="submit" name="submit" class="">Hoàn Thành</button>
											</div>
										<?php
										}
										?>
									<div class="b-search--button">
										<button type="submit" name="xoa" class="">Xóa đơn hàng</button>
									</div>
								</ul>
							</form>
					...
					<div class="button-set">
						<a href="ad_order_history.php" class="left">« Quay về mục lịch sử đơn hàng</a>
